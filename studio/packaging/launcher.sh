...
CLOUDFLARED_DIR="$RESOURCES/cloudflared"
...
# $AWSCLI_DIR + $CLOUDFLARED_DIR last so `aws` and `cloudflared` resolve
# to our bundled binaries when the user doesn't have them installed — but
# a system-installed version (e.g. from Homebrew) still wins because it
# comes earlier in $PATH.
export PATH="$NODE_BIN:$LOCAL_BIN:$PATH:$AWSCLI_DIR:$CLOUDFLARED_DIR"
