...
> The Worker only serves `POST /share` (the Pages Direct-Upload proxy). The
> multiplayer `/project`, `/join`, and `/rendezvous` routes and their
> `KV_RENDEZVOUS` namespace were removed when the multiplayer feature was
> pulled for a rebuild — there is no KV namespace to provision.
