...
**Target file:** write the lifted frame to \`${targetFile}\`. Create the directory if needed. Do NOT register the file in a router, data layer, or i18n — this is a translation experiment, not a feature ship. The lift's deliverables are TWO things, both of which must survive when you finish: (1) the lifted frame file, and (2) a LIVE, CLICKABLE Storybook showing it rendered — the reviewer opens the iframe URL in their own browser and clicks around. The manifest's \`<render_harness>\` has you create a validation story and boot Storybook to produce that live render; it is an in-scope verification artifact, NOT a forbidden codebase change. Don't skip the render and report it as a gap. When done, LEAVE STORYBOOK RUNNING and KEEP the story — do not tear it down.
...
10. VERIFY BY RENDERING, AND LEAVE IT UP. If the manifest has a \`<render_harness>\`, follow its \`<story_scaffold>\` verbatim: create the validation story, boot Storybook, navigate the \`<iframe_url>\`, and check each \`<check>\` against LIVE \`getComputedStyle\` output. A class string existing in the file (grep) does NOT prove it paints — many tokens are app-scoped or theme-gated and fall through to transparent. Do not substitute grep for a live render and call the lift verified. When the checks pass, do NOT clean up: KEEP the story file and LEAVE STORYBOOK RUNNING so the reviewer can open the URL themselves. The only housekeeping allowed: if you moved a pre-existing broken story aside to get the build green, restore it — unless restoring re-breaks the running build, in which case leave it parked and say so (a clickable server beats a tidy story dir).
...
**When done:** the lift is NOT done until you have rendered it live, confirmed the \`<render_harness>\` checks against computed styles, and LEFT STORYBOOK RUNNING (step 10). Then print a short summary — put the clickable iframe URL on its OWN LINE first (plus the port and a one-line how-to-stop-it note) so the reviewer can open it immediately, then confirm the render passed (and what you read), how many TODOs you left, what each represents, and anything the manifest told you but didn't give you enough to act on. If you genuinely could not render, say so explicitly and name what blocked it — do not present a grep-only check as verification.
