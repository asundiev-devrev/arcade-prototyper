...
- **Select** — single-value. `value`/`defaultValue` are STRINGS. NO `multiple` prop. For multi-value use `MultiSelect` (checkbox list + selection summary) or `Combobox` (type-to-filter + chips) — both are real components. (react-select index.d.ts: defaultValue?: string)
- **MultiSelect** — TRUE multi-select dropdown. `options: {value,label,disabled?}[]`, `value`/`defaultValue` are STRING ARRAYS, `onValueChange(string[])`. Also `label`/`labelStyle`/`placeholder`/`helperText`/`error`/`size`. Use this instead of approximating multi-select with a segmented control.
- **Combobox** — multi-select WITH type-to-filter. Same `options`/`value: string[]`/`onValueChange` shape as MultiSelect, plus `inputValue`/`onInputValueChange` for the filter text. Selections render as chips.
- **Tabs** — `value`/`defaultValue` are STRINGS. No multi-value. (react-tabs index.d.ts: defaultValue?: string)
- **SegmentedControl** — the row of mutually exclusive pills (v1 called this `ToggleGroup`). Compound — `<SegmentedControl.Root type="single">` + `<SegmentedControl.Item value="…">`. Supports BOTH `type="single"` (`value`/`defaultValue`: string) and `type="multiple"` (`value`/`defaultValue`: string[]) — via `type`, NOT a `multiple` prop. (react-toggle-group index.d.ts: union on `type`)
- **ToggleGroup** — NOT a segmented control — a vertical list of labelled toggle rows. NOT compound: there is NO `.Root` (writing `<ToggleGroup.Root>` crashes the frame). `<ToggleGroup aria-label="…">` (aria-label REQUIRED) wrapping `<ToggleGroup.Item label="…" description="…" pressed onPressedChange={…} />`. Items take `label`, NOT `value`. For pill-style segmented options use `SegmentedControl`.
- **Accordion** — same `type` union as SegmentedControl: `type="single"` (`value`/`defaultValue`: string) or `type="multiple"` (`value`/`defaultValue`: string[]). Multiple open panels IS supported — via `type="multiple"`, NOT a `multiple` prop. (react-accordion index.d.ts: union on `type`)
...
- **SearchInput** — packaged search field — leading magnifying glass + clear button. `value`/`defaultValue` STRINGS, `onValueChange(string)`, `onSearch(string)` on Enter, `placeholder`. Placeholder-only (no label). Use instead of hand-dressing an `Input` with a search icon.
- **NumberField** — numeric field with +/- steppers. `value` is `number | null` (null = empty), `defaultValue: number`, `onValueChange(number|null)`, plus `min`/`max`/`step` clamping and `label`/`helperText`/`error`.
...
**When NOT to use this:**
- Do NOT import `Markdown` from `arcade/components` for a chat
...
- Do NOT hand-roll `<pre><code>` for a fenced code block inside
