...
export function buildEditContextBlock(frameSlugs: string[]): string {
  ...
  return [
    ...
    "- Make the SMALLEST change that satisfies the request. Use the Edit tool with a",
    "  tight anchor and change only the lines that must change — do NOT rewrite or",
    "  re-emit the whole file. Reserve Write for a brand-new file or when Edit truly",
    "  cannot find a unique anchor after you widen the context.",
    "",
