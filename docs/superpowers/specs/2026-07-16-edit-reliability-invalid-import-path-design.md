...
| File | Change |
|---|---|
| `studio/server/hooks/validateArcadeImports.mjs` | add `isResolvableArcadeSpecifier(spec)` (exact `arcade`/`arcade/components` + PREFIX `arcade-studio(/*)`/`arcade-prototypes(/*)` + `arcade-user/<name>`); add `extractImportSpecifiers(source)` (NEW scan anchored on the `import`/`export … from "…"` + side-effect `import "…"` STATEMENT shape — NOT a loose `\bfrom` which would phantom-match prose/JSX/template string bodies that stripComments keeps; NOT reused from parseImports/collectDefinedIdentifiers) + `detectInvalidArcadePaths(specifiers)`; call it in `main()` alongside `validateImports`; union violations into one exit-2; a specific correct-path message + formatter. Comment cross-refs vite.config.ts alias list + the exact-vs-prefix distinction as the authority. |
