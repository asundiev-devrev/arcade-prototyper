...
- The table lists, for the props-bearing primitives agents reach for (Select, ToggleGroup, Tabs, Switch, Input, Button — start with the ones already seen misused), the real prop surface + notable ABSENCES + value shapes. Entries must be TYPE-ACCURATE (verified against the real Radix types — do NOT lump them):
  - **`Select` — single-value; `value`/`defaultValue` are STRINGS; NO `multiple` prop; the kit has NO multi-select Select control.**
  - **`Tabs` — `value`/`defaultValue` are STRINGS; no multi-value.**
  - **`ToggleGroup` — supports BOTH: `type="single"` (`value`/`defaultValue`: string) and `type="multiple"` (`value`/`defaultValue`: string[]). There is NO `multiple` prop — multi-select is expressed via `type="multiple"`.** (Do NOT write "ToggleGroup has no multi-select" — it does, via `type`.)
...
**Correction:** the Deviations contract is NOT in `visualEditPreamble.ts`/`editContext.ts` (rev-1's guess — both merely mention it in passing; the picker preamble contains the word "Deviations" ZERO times, so rev-1's Open-Q2 premise was false). It is authored in **`studio/templates/CLAUDE.md.tpl` §"Response shape" (lines 68-112)** — verified the file is at `studio/templates/`, NOT `studio/prototype-kit/templates/` (a different real dir holding composite `.tsx`) — rendered into each project's `CLAUDE.md` and loaded from the spawn cwd — the "obeyed harder" instruction source (`editContext.ts`, `claudeCode.ts:19`).
...
- Add a fourth hook `studio/server/hooks/validateComponentProps.mjs`, registered alongside the others (`claudeCode.ts:276`, `Write|Edit` matcher). It AST-walks the written frame source for a **bounded denylist of known-invented props/shapes** on kit components. **Detection MUST key on the capitalized member-expression tag (`Select.Root` / `ToggleGroup.Root` / `Tabs.Root`)** — NOT a bare attribute scan — so a legit native lowercase `<select multiple>` / `<input multiple>` (real HTML) is never flagged.
- **The denylist is NOT uniform across the three components — this is the load-bearing correction (verified against the real Radix types):**
  - **`Select.Root` and `Tabs.Root`:** genuinely string-only (`react-select` has NO `multiple`, `react-tabs` `defaultValue?: string`). Flag UNCONDITIONALLY: a `multiple` prop → invalid; an ARRAY literal for `value`/`defaultValue` → invalid (wants a string).
  - **`ToggleGroup.Root`:** is a DISCRIMINATED UNION on `type` (`react-toggle-group`: `type="single"` → `value/defaultValue: string`; `type="multiple"` → `value/defaultValue: string[]`). So `<ToggleGroup.Root type="multiple" defaultValue={["a","b"]}>` is CORRECT and must NOT be flagged. Rules for ToggleGroup: flag a `multiple` PROP (it doesn't exist — the real API is `type="multiple"`), and flag an ARRAY `value`/`defaultValue` ONLY when the sibling `type` is `"single"`, absent, or unknown — NEVER when `type="multiple"`. The self-correct message points at `type="multiple"`, not "the kit has no multi-select."
- On a hit → `exit(2)` with a component-specific self-correct message (Select: "no `multiple` prop; `defaultValue` is a string, not an array — the kit has no multi-select Select; leave it single or approximate + note as a Deviation." ToggleGroup: "no `multiple` prop — use `type='multiple'`; an array `defaultValue` needs `type='multiple'`."). Fail open; frame-files-only scope; mirror the shipped hooks' structure.
- **Scope honesty (Open Q1):** this is a targeted denylist, NOT a general prop typechecker (that's keystone-adjacent). It catches the repro + its class and grows by adding rows, exactly like `kitMappings`/the token seed. State plainly: it does NOT catch every possible invented prop — un-enumerated hallucinations still pass, and an aliased kit component (`import { Select as Dropdown }`) is a known false-negative (tag-name keying). That's the accepted v1 boundary; Parts A+B (prevention + honest-report discipline) cover the long tail softly.
...
| `studio/templates/CLAUDE.md.tpl` (§"Response shape", ~`:73-75`) | Part B: reword so `### Deviations … None.` is a VERIFIED claim, not the appended default — `None.` only when every component/prop/token used is real AND the literal ask was fulfilled; an invented prop / absent capability / unfulfilled ask is a Deviation, never None. Land it as "build the closest thing + flag it" (NOT ask/stall — the generator is headless, `claudeCode.ts:204`), consistent with the existing `:14` "request is LAW … never refuse, never stall" and `:101` "uncertainty counts as a deviation." (Do NOT touch `deviationsContract.ts` for prop-detection — it's narration+hash-only, structurally can't see the written prop.) |
| `studio/server/hooks/validateComponentProps.mjs` (NEW) + `studio/server/claudeCode.ts` (`:276` register a 4th PostToolUse `Write|Edit` hook) | Part C: AST-walk the written frame source (read from disk + `ts.createSourceFile`, mirroring `validateArcadeImports.mjs`). Key on the capitalized member-tag (`Select.Root`/`Tabs.Root`/`ToggleGroup.Root`), NOT a bare attribute scan. Rules: `multiple` prop on any of the three → invalid; array `value`/`defaultValue` on Select/Tabs → invalid; array `value`/`defaultValue` on ToggleGroup → invalid ONLY when sibling `type`≠`"multiple"`. Hit → `exit(2)` component-specific self-correct. Fail open; frame-files-only. |
| `studio/__tests__/server/hooks/validateComponentProps.test.ts` (+ manifest + template tests) | Part C positive: `<Select.Root multiple>` → exit 2; `<Select.Root defaultValue={["x"]}>` → exit 2 (array-where-string); `<ToggleGroup.Root multiple>` → exit 2 (use type). Part C EXEMPTIONS (must be exit 0 — the false-alarm guards): `<Select.Root defaultValue="x">`; native `<select multiple>` and `<input multiple>` (lowercase HTML — NOT flagged); **`<ToggleGroup.Root type="multiple" defaultValue={["a","b"]}>` (valid multi-toggle — NOT flagged)**; `<ToggleGroup.Root type="single" defaultValue="a">`. Fail-open on unreadable. Part A drift-guard: the `PRIMITIVE_CAPABILITIES` facts match the REAL types — Select/Tabs string-only + no `multiple`; ToggleGroup union `type` string vs string[] (resolve `@xorkavi/arcade-gen` `.d.mts` → the Radix `.d.ts`; transitive resolution is nontrivial, Open Q3) — at minimum assert component NAMES are real exports + a comment citing the radix `.d.ts` line per fact. |
