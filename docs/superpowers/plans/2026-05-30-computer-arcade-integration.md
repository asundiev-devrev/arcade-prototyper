...
The canvas iframe runs with `sandbox="allow-scripts"` only (no `allow-forms`). **Already verified live (2026-06-02) outside Computer**: under an `allow-scripts`-only iframe, Annotate → click element → type comment → Add → pin persists + "Send to Computer" appears, all working (the composer is a script-driven `<textarea>`, no HTML form submit). This step is now just a regression re-confirm inside Computer's *actual* canvas: click **Annotate**, click an element, type a comment, commit — the pin must persist. Only if it unexpectedly fails here does the sandbox need `allow-forms` in `html-artifact-renderer.tsx` (a devrev-web change + security review) — do not loosen the sandbox ad hoc.
...
5. ~~Whether Computer's canvas sandbox (`allow-scripts` only) lets the comment composer commit.~~ **RESOLVED (verified live, 2026-06-02).** Ran the full pack → make-interactive pipeline on a real arcade frame, served it under an `allow-scripts`-only iframe (matching Computer's canvas), drove it with Playwright: clicked Annotate, clicked an element, **typed a comment, the "Add" button enabled, committed, a pin landed and "Send to Computer" appeared.** No `allow-forms` needed — the composer is a script-driven textarea as the code read predicted. Task 5.1 Step 3b is now a regression re-confirm inside Computer's actual canvas, not an open risk.
6. How `DEVREV_TOKEN` reaches the publisher when the user shares (Task 2.1 publish block / Task 5.1 Step 3c). ui-publisher reads it from env; confirm the Computer agent's Bash environment has a DevRev PAT, or whether `pat-manager` (also a Ribhu skill) should be the supported fallback. Sharing is opt-in, so this never blocks the core generate→comment loop. **GOTCHA (cost an investigation 2026-06-02):** not every `DEVREV_*` token works. The agent/MCP-gateway token (`DEVREV_API_KEY`, JWT `aud: ["janus"]`) is rejected by the REST gateway ui-publisher calls — `dev-users.self` returns HTTP 401 / "Could not resolve calling user". ui-publisher needs a **REST PAT** (DevRev → Settings → Account → Tokens), exported as `DEVREV_TOKEN` (or `DEVREV_PAT`). Decode the JWT `aud` to tell them apart before assuming "token expired".
7. ~~Whether `ui-publisher` renders our interactive `prototype.html` byte-for-byte.~~ **RESOLVED (verified live, 2026-06-02).** Published a real interactive prototype via `publisher.py --access public`, opened the returned 7-day signed URL in a fresh browser: renders the full arcade prototype with the Annotate/View bar + Comments, identical to local. ui-publisher hosts the file verbatim (raw S3 object, `response-content-type: text/html`) — no re-flatten, no tighter sandbox. (Note: in `public` mode the link is the raw S3 object, not the `devrev-artifact-viewer.vercel.app` wrapper; the viewer wrapper is the `personal`/`internal` path and wasn't separately tested.)
```
