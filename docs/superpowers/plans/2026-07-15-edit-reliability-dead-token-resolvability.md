...
- **The resolvable set is a UNION; neither source is complete alone.** The kit-CSS half (`loadTokenNames()` → `styles.css`) carries the 7 `*-on-prominent`/`--bg-neutral-transparent` tokens that lossy ADS-name normalization drops; the ADS seed carries the expressive-orange family the kit lacks. Both halves are mandatory.
...
// Names normalized to the kit custom-property form (sans leading --), Light value.
// Provenance: Arcade Design System, Figma file a2uKnm88LxRXEWAL1kOqeQ.
...
export const ADS_COLOR_SEED = {
  "bg-expressive-orange-subtle": "#FCECD2",
  "bg-expressive-orange-medium": "#FFDAA3",
  "bg-expressive-orange-prominent": "#D14600",
  "fg-expressive-orange-prominent": "#FF7924",
  "bg-expressive-blue-subtle": "#E7FBFF",
  "bg-expressive-yellow-subtle": "#FFFA9B",
  "bg-neutral-subtle": "#211E2026",
  "surface-shallow": "#FAF9F9",
  "fg-neutral-medium": "#737072",
  // … ADD EVERY captured ADS color row here (≈90 total: all BG/FG/Surface/Stroke
  // Expressive {color}/{Subtle,Medium,Prominent}, semantic {Alert,Info,Success,
  // Warning,Intelligence}, Neutral, and the onProminent set).
};
...
The complete row list is in the rev-5 spec's source capture — transcribe ALL of them, not just the sample. Normalize each name with the same rules as `figmaVarNameToKitToken` (lowercase, `/`→`-`, whitespace→`-`, collapse `-`, drop leading `--`).
...
```javascript
  ...
  if (isFrameFile(toolInput?.file_path)) {
    // UNION: kit CSS (load-bearing — carries *-on-prominent) ∪ ADS seed ∪
    // project overrides ∪ same-file local defs. Fail open: empty → skip.
    const resolvable = new Set(tokenNames);                        // kit CSS
    for (const k of Object.keys(ADS_COLOR_SEED)) resolvable.add(k); // ADS seed
